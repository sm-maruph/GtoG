...
export default function CompanyDonut({ rows, valueKey, title, formatter=(v)=>v, onCompanyClick }){
  ...
  return <div className="ins-donut-card"><h3>{title}</h3><div className="ins-donut-wrap">
    ...
    <div className="ins-legend">{rows.map((row,index)=><button type="button" key={row.company} onMouseEnter={()=>setHover(index)} onMouseLeave={()=>setHover(null)} onClick={()=>onCompanyClick?.(row.company)}><span className="ins-legend-dot" style={{background:PALETTE[index%PALETTE.length]}}/><span>{row.company}</span><strong>{formatter(row[valueKey])}</strong></button>)}</div>
  </div></div>;
}
